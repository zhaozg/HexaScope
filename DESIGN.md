...
### ADR-005：为什么对 `demo-report.json` 做结构 + 一致性双重校验

**状态**：已采纳

**背景**：`frontend/demo-report.json` 是站点内置的演示数据（`?user=demo` 在 GitHub Pages 上
离线可用）。它是**静态文件**，不会随评分算法自动更新：一旦维度名、权重或报告结构变更，
演示报告会静默展示旧结构（缺字段、维度名不符，甚至综合得分与算法脱节），而演示报告
正是新用户的第一印象（PLAN.md P0-2）。

**决策**：新增 `scripts/reportSchema.ts`（纯函数）+ `scripts/validate-demo-report.ts`（CLI），
并在 `ci.yml` 中作为门禁运行（`bun run validate:demo`）。校验分两层：

1. **结构层**：字段存在性、类型、取值范围（得分 0-100）、10 项红牌编号 1-10 不重复、
   已触发红牌必须有 `detail`、可选字段 `coverage` / `insights` 出现时结构必须正确；
2. **一致性层**：维度键名与顺序必须等于 `scoreCalculator.DIMENSION_NAMES`、权重必须等于
   `DIMENSION_WEIGHTS`、`overallScore` 必须等于用**同一算法**重算的结果（容差 1e-6）、
   `coverage` 计数与 `dimensions` 自洽。

**为什么不用 zod**：AGENTS.md §2 偏好零新增依赖；维度名与权重需要复用评分引擎的常量做
一致性比对，手写校验器能直接 import 这些常量并给出版本化的中文错误信息
（`dimensions[2].weight — 权重应为 0.2，实际为 0.5（算法权重已变更？）`），无需额外依赖。

**失败即报错**：校验不通过时输出全部问题（而非首个）并以退出码 1 结束，CI 因此在部署前拦截，
错误信息提示「请更新 demo-report.json 以匹配当前 schema」。

**代价**：演示报告需与算法同步更新（这是有意为之的约束）；新增 1 个 CI 步骤（秒级，无额外分钟数压力）。

---

### ADR-006：为什么弱化项用“中性配色”而非 `opacity`

**状态**：已采纳

**背景**：未触发的红牌（`.flag-item`）与数据不足的维度原先用 `opacity: .55 / .6` 弱化。
但透明度会把文字颜色与背景混色，实测对比度降到 **2.62:1**，远低于 WCAG AA 要求的 4.5:1
（Lighthouse `color-contrast` 直接不通过），对低视力用户不可读。

**决策**：改用中性配色表达弱化（未触发项用 `--text-subtle` 而非整体降透明度），
并新增两类可访问性约束：

- 按钮底色使用 `--brand-strong`（#1f6feb，白字对比度 4.63:1），而非 `--brand`（3.75:1）——
  `--brand` 仍用于链接文字（对页面底色 5.05:1）；
- 正文段落内的链接加下划线，不依靠颜色单独传达语义（WCAG 1.4.1）；
- 表单控件必须有真实 `label`（`.sr-only`），`<html lang>` 由 `site.yaml` 的 `language: zh-CN` 产出。

**验收**：Lighthouse 可访问性（首页 / 演示页）均 **100**；375px 宽无横向滚动。

**代价**：弱化效果略弱于半透明，但换来了可读性；新增 `--brand-strong` 令牌需与 `--brand` 区分使用。

---

### ADR-007：安装入口为何必须常驻首屏，而非折叠在「原理说明」里

**状态**：已采纳

**背景**：站点上线后实测发现一个漏斗断点——**前端代码中不存在任何指向安装页的链接**。
「安装 HexaScope GitHub App」只作为纯文本出现在默认折叠的 `<details>` 中（且不可点）；
首屏可见按钮只有「生成雷达图」与「查看演示报告」：前者对**尚未安装的用户必然失败**，
而失败页以「返回首页」收尾，形成闭环死胡同（用户既不知先装 App，也不知要评论 `/evaluate`）。

**决策**：把安装入口提升为站点的一等公民，并让「查询失败」也能自服务：

1. **首屏主 CTA**：首页标题下新增主行动区，实心绿主按钮「安装 HexaScope App」+ 次级描边按钮
   「查看演示报告」，主次分明（绿=安装、蓝=查询/演示）；微文案交代「只读权限 · 免费开源 · 约 1 分钟」。
2. **常驻入口**：顶栏与页脚各加一个安装链接，任何滚动位置都能找到第一步。
3. **三步流程常驻可见**：原折叠的 `<details>` 改为三张固定卡片（安装 / 发起评估 / 查看报告），
   每步都带可点链接；第 2 步直接指向**预填 `/evaluate <用户名>` 的新建 Issue**。
4. **查询入口降级并改名**：查询表单挪到三步之后，标题改为「已有报告？直接查询」，
   提交按钮由「生成雷达图」改为「查看报告」（它并不生成，旧文案制造错误预期）。
5. **空态可自服务**：查询不到报告时，页面直接给出「安装 App」+「打开预填好的评估 Issue」两个按钮
   （Issue 正文已填入 `/evaluate <当前用户名>`），替换原先的「返回首页」死胡同。

**单一事实来源**：安装地址与评估 Issue 预填链接收敛为 `frontend/ui/entry.html` 顶部的
`APP_INSTALL_URL` / `REPO_URL` / `buildEvaluateIssueUrl()`，两个组件共用；
`tests/frontend-home-cta.test.ts` 断言站点 URL 与 README / DESIGN 记录的规范地址完全一致，
并断言预填指令与 `analyze.yml` 解析的 `/evaluate` 同名，防止两处漂移。

**顺带修正的对比度问题**：新主按钮绿取 `#238636`（白字 **4.63:1**，达 WCAG AA）。
注意 GitHub 原版 hover 绿 `#2ea043` 白字仅 **3.37:1**，故 hover 改用更深一档的 `#1a7f37`（**5.08:1**）；
次级按钮 hover 由「半透明蓝叠加」改为实心 `--brand-strong`（叠加会把文字对比度压到 4.01:1）。
上述取值已写入 `tests/frontend-home-cta.test.ts` 的对比度断言（令牌改动越界即失败）。

**验收**：Lighthouse 可访问性 首页 **100** / 空态页 **100**；375 / 414 / 768 / 1440px 视口均无横向滚动，
主按钮高度 47px（≥ 44px 触控目标），窄屏下主/次按钮各占满整行。

---

### ADR-008：组件 `<script>` 注释内禁止出现反引号（Nuekit 编译器陷阱）

**状态**：已采纳

**背景**：为满足 ADR-007 的自服务空态，需要在组件方法里拼出 Issue 预填链接，于是按惯例写了
带反引号的注释。构建**不报错**，但产物 `ui/entry.html.js` 中 `dashboard` 组件的 `<script>`
被编译成一整段被转义的字符串，浏览器解析时抛 `Unexpected escape sequence`，
**整站白屏（含顶栏与首页）**。

**根因**：Nuekit 2.0.0-beta.2 编译组件脚本时会按模板字符串处理脚本内容，注释中的反引号
会错配界符，破坏**同一文件内后续组件**的脚本产物；构建期无任何告警，只有运行时白屏。
实测：同一文件出现 3 处注释反引号时 `dashboard` 脚本必坏；逐处移除后产物恢复正常。

**决策**：`frontend/` 的组件脚本及其注释内一律不使用反引号——需要引用指令时改用「」引号，
反引号只留给模板字符串自身。并以两条测试固化（`tests/frontend-home-cta.test.ts`）：

1. `frontend/ui/entry.html` 中不存在以 `//` 开头且含反引号的行；
2. 每个 `<script>` 块内反引号数量为偶数（模板字符串可配对）。

**代价**：失去在注释里直接写 Markdown 行内代码的便利；换取的是「构建期无报错但线上白屏」
这类隐性故障不再发生。若未来 Nuekit 修复该行为，可放宽此约束。
